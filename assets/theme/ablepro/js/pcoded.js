...
window.feather = require('feather-icons/dist/feather');
import SimpleBar from "simplebar";

// ----- [ Template Configuration ] start ----- //
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    // collapse like accordion start
    $(".acollapsetrigger").click(function () {
        const showCollapseContent = $(this).attr("href");
        setTimeout(function () {
            $.each($('div>.collapse'), function () {
                const myCollapseSelector = '#' + $(this).attr("id");
                if (myCollapseSelector !== showCollapseContent) {
                    $(myCollapseSelector).removeClass("show");
                } else {
                    $(myCollapseSelector).addClass("show");
                }
            });
        }, 100);
    });

    // collapse like accordion end
